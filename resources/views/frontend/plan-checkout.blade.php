         ...
         <div class="col-lg-8 plan-pay">
            <div class="card">
               <div class="card-body rounded">
                  ...
                  @if ($plan->price == 0)
                  <div class="p-md-4 payment-info text-center">
                     <h6 class="mb-2">@lang('Free Subscription. Just click on activate button. It will automatically activate')</h6>
                  </div>
                  <form action="{{ route('checkout.free.submit') }}" method="POST">
                     @csrf
                     <input type="hidden" name="user_id" value="{{ auth()->id() }}">
                            ...
                            <input type="hidden" name="amount" id="amount" value="{{ rootAmount($plan->price) }}">

                            <div class="cmn-btn plan-btn my-3">
                              <button type="submit" class="btn btn-spay w-100">{{__('Activate Now')}}</button>
                          </div>
                  ...
                  <div class="p-md-4">
                     ...
                     <div class="tab-content" id="pills-tabContent">
                        {{-- tab panel one start --}}
                        <div class="tab-pane fade show active my-2" id="pills-home" role="tabpanel" aria-labelledby="pills-home-tab">
                        ...
                        <form id="mercadopago" method="POST" class="payment-form" action="">
                           ...
                           <div class="col-sm-12">
                              <div id="card-view" class="col-md-12 d-none">
                                 <div class="row gy-3">
                                    <div class="col-md-6">
                                       <input type="text" class="form-control card-elements" name="cardautho" placeholder="@lang('Card Number')" autocomplete="off"  autofocus oninput="validateCard(this.value);"/>
                                       <span id="errCard"></span>
                                    </div>
                                    <div class="col-lg-6 cardRow">
                                       <input type="text" class="form-control card-elements" placeholder=" @lang('Card CVC') " name="cardCVC" oninput="validateCVC(this.value);">
                                       <span id="errCVC"></span>
                                    </div>
                                    <div class="col-lg-6">
                           ...
                           <div class="col-sm-12">
                              <div id="mergado-view" class="col-md-12 d-none">
                                 <div class="row ">
                                    <div class="row gy-3">
                                       <div class="col-lg-6">
                                          <input class="form-control mergado-elements" type="text" placeholder="@lang('Credit Card Number')" name="cardNumber" id="cardNumber" data-checkout="cardNumber" onselectstart="return false" autocomplete="off" />
                                       </div>
                                       <div class="col-lg-6">
                                       ...
                                       <div class="col-lg-6">
                                          <input class="form-control mergado-elements" type="text" id="cardExpirationMonth" name="cardExpirationMonth" data-checkout="cardExpirationMonth" placeholder="@lang('Expiration Month')" autocomplete="off" />
                                       </div>
                                       <div class="col-lg-6">
                                          <input class="form-control mergado-elements" type="text" id="cardExpirationYear" name="cardExpirationYear" data-checkout="cardExpirationYear" placeholder="@lang('Expiration Year')" autocomplete="off" />
                                       </div>
                                       <div class="col-lg-6">
                                          <input class="form-control mergado-elements" type="text" id="cardholderName" name="cardholderName" data-checkout="cardholderName" placeholder="@lang('Card Holder Name')" />
                                       </div>
                                       <div class="col-lg-6">
                                          <div class="col-lg-12">
                                             <div class="row">
                           ...
                           <div class="d-flex justify-content-between">
                            <div class="col-sm-5">
                                <div class="input-group my-4">
                                    <span class="input-group-text" id="basic-addon1">{{ $defaultCurrency->sign }}</span>
                                    <input disabled type="text" class="form-control" placeholder="Plan Price"  value="{{ rootAmount($plan->price) }}" >
                                  </div>
                              </div>

                            <div class="col-sm-6">
                                <select id="method" name="method" required class="form-control @error('method') is-invalid @enderror w-100 my-4">
                           ...
                           <div class="cmn-btn plan-btn my-3">
                                <button type="submit" class="btn btn-spay w-100">{{__('Pay Now')}}</button>
                            </div>
                        ...
                        {{-- Tab panel one end --}}
                        {{-- tab panel two start --}}
                        <div class="tab-pane fade my-2" id="pills-profile" role="tabpanel" aria-labelledby="pills-profile-tab">

                           
                       
                        @includeIf('includes.flash')
                        <form id="subscribe-form" class="pay-form" action="{{ route('wallet.plan.submit') }}" method="POST">
                            @csrf

                            <div class="d-flex justify-content-between">

                              <div class="col-sm-6">
                                 <label for="">@lang('Wallet Balance:')</label>
                                    <div class="input-group my-4">
                                       <span class="input-group-text" id="basic-addon1">{{ $defaultCurrency->sign }}</span>
                                       <input disabled type="text" class="form-control" placeholder="User Balance"  value="{{ rootAmount(Auth::user()->balance) }}" >
                                       </div>
                                 </div>

                                 <div class="col-sm-5">
                                    <label for="">@lang('Plan Price:')</label>
                                       <div class="input-group my-4">
                                          <span class="input-group-text" id="basic-addon1">{{ $defaultCurrency->sign }}</span>
                                          <input disabled type="text" class="form-control" placeholder="Plan Price" name="amount"  value="{{ rootAmount($plan->price) }}" >
                                          </div>
                                    </div>
                              </div>
                   ...
                   <div class="card-body rounded bg--base text-white">
                       <div class="p-4 pb-0">
                           ...
                           <ul class="list--group">
                               <li>
                                   <div>{{ $plan->title }}</div>
                                   <div>{{ $defaultCurrency->sign }}{{ rootAmount($plan->price) }}</div>
                               </li>
                               <li>
                                   <div>
                                       @lang('Plan Type')
                                   ...
                                   <div>
                                       @if ($plan->plan_type == 'yearly')
                                           @lang('Yearly')
                                       @elseif($plan->plan_type == 'monthly')
                                           @lang('Monthly')
                                       @elseif($plan->plan_type == 'life_time')
                                             @lang('Life Time')
                                       @endif
