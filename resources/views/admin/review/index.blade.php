...
<div class="card">
	...
	<h5 class=" mb-0 text-gray-800 pl-3">{{ __('Testimonial Section') }}</h5>
	<ol class="breadcrumb py-0 m-0">
		<li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{ __('Dashboard') }}</a></li>

		<li class="breadcrumb-item"><a href="{{ route('admin.review.index') }}">{{ __('Testimonial Section') }}</a></li>
	</ol>
	</div>
</div>
				  ...
				  <div class="form-group">
					...
					<div class="wrapper-image-preview">
						<div class="box full-width">
							<div class="back-preview-image" style="background-image: url({{ $ps->testimonial_banner ? asset('assets/images/'.$ps->testimonial_banner) : asset('assets/images/placeholder.jpg') }});"></div>
							<div class="upload-options">
								<label class="img-upload-label full-width" for="img-upload"> <i class="fas fa-camera"></i> {{ __('Upload Picture') }} </label>
								<input id="img-upload" type="file" class="image-upload" name="testimonial_banner" accept="image/*">
							</div>
						</div>
					</div>
				</div>
