...
<div class="card">
  <div class="d-sm-flex align-items-center justify-content-between py-3">
    <h5 class=" mb-0 text-gray-800 pl-3">{{ __('Edit Testimonial') }} <a class="btn btn-primary btn-rounded btn-sm" href="{{route('admin.review.index')}}"><i class="fas fa-arrow-left"></i> {{ __('Back') }}</a></h5>
    <ol class="breadcrumb py-0 m-0">
        <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{ __('Dashboard') }}</a></li>
        <li class="breadcrumb-item"><a href="{{route('admin.review.index')}}">{{ __('Testimonial') }}</a></li>
    </ol>
  </div>
</div>
  ...
  <div class="card mb-4 mt-3">
    <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
      <h6 class="m-0 font-weight-bold text-primary">{{ __('Add New Testimonial Form') }}</h6>
    </div>
    ...
    <div class="card-body">
      ...
      <form class="geniusform" action="{{route('admin.review.update',$data->id)}}" method="POST" enctype="multipart/form-data">
          ...
          <div class="form-group">
            <label for="subtitle">{{ __('Video URL') }}</label>
            <input type="text" class="input-field" name="video_url" placeholder="{{ __('Video URL here') }}" required="" value="{{ $data->video_url }}">
          </div>
