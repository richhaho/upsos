        ...
        <div class="card-body">
          ...
          <form class="geniusform" action="{{ route('admin.social.update') }}" method="POST" enctype="multipart/form-data">
              ...
              <div class="form-group">
                <label for="inp-title">{{  __('Facebook Login')  }} </label>
                <div class="frm-btn btn-group mb-1">
                    <button type="button" class="btn btn-sm btn-rounded dropdown-toggle btn-{{ $data->f_check == 1 ? 'success' : 'danger' }}" data-toggle="dropdown"
                      aria-haspopup="true" aria-expanded="false">
                      {{ $data->f_check == 1 ? __('Activated') : __('Deactivated')}}
                    </button>
                    <div class="dropdown-menu">
                      <a class="dropdown-item drop-change" href="javascript:;" data-status="1" data-val="{{ __('Activated') }}" data-href="{{ route('admin.social.facebookup',1) }}">{{ __('Activate') }}</a>
                      <a class="dropdown-item drop-change" href="javascript:;" data-status="0" data-val="{{ __('Deactivated') }}" data-href="{{ route('admin.social.facebookup',0) }}">{{ __('Deactivate') }}</a>
                    </div>
                  </div>
            </div>
