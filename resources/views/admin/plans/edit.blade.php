        ...
        <form class="geniusform" action="{{route('admin.plans.update',$data->id)}}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select name="plan_type" class="form-control" id="plan_type">
                  <option value="monthly" {{ $data->plan_type == 'monthly' ? 'selected' : '' }}> {{__('Monthly')}} </option>
                  <option value="yearly" {{ $data->plan_type == 'yearly' ? 'selected' : '' }}> {{__('Yearly')}} </option>
                  <option value="life_time" {{ $data->plan_type == 'life_time' ? 'selected' : '' }}> {{__('Life Time')}} </option>
                </select>
            </div>

            <div class="form-group">
                <label for="price">{{ __('Price') }} ({{$currency->name}})</label>
                <input type="number" class="form-control" id="price" name="price" placeholder="{{ __('Plan Price') }}" min="0" step="0.01" value="{{ showAdmininputPrice($data->price) }}">
            </div>

            <div class="form-group">
                <label for="connect">{{ __('Connect') }}</label>
                <input type="number" class="form-control" id="connect" name="connect" placeholder="0" min="0" value="{{ $data->connect }}">
            </div>

            <div class="form-group">
              <label for="job">{{ __('Apply Job Limit') }}</label>
              <input type="number" class="form-control" id="job" name="job" placeholder="0" min="0" value="{{ $data->job }}">
              <span>@lang('Apply Job Limit')</span>
          </div>

          <div class="form-group">
            <label for="service">{{ __('Create Service Limit') }}</label>
            <input type="number" class="form-control" id="service" name="service" placeholder="0" min="0" value="{{ $data->service }}">
          </div>
