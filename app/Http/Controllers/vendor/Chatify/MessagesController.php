...
class MessagesController extends Controller
{
    ...
    public function idFetchData(Request $request)
    {
        ...
        if($fetch){
   			...
   			if(($fetch)->photo === null)
			{
				$userAvatar = '../assets/images/avatars/avatar.png';
			}
			else if (($fetch)->is_provider == 1)
			{
				$userAvatar = (($fetch)->photo);
			}
			else
			{
				$userAvatar = ('../assets/images/avatars/' . ($fetch)->photo);
			}
		}
    ...
    public function sharedPhotos(Request $request)
    {
        ...
        for ($i = 0; $i < count($shared); $i++) {
            $sharedPhotos .= view('Chatify::layouts.listItem', [
                'get' => 'sharedPhoto',
                'image' => '../project/storage/app/public/attachments/'. $shared[$i],
            ])->render();
        }
        // send the response
        return Response::json([
